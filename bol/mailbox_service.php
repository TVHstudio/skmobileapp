...
class SKMOBILEAPP_BOL_MailboxService extends SKMOBILEAPP_BOL_Service
{
    ...
    public function getConversations($userId, $limit)
    {
        ...
        {
            $conversations[$avatar->userId]['user']['avatar'] = $this->getAvatarData($avatar, false);
        }
...
//            throw new Exception($language->text('mailbox', 'chat_message_empty'));
    ...
    public function getMessageData($userId, $conversationId, MAILBOX_BOL_Message $message)
    {
        ...
        $processedText = trim(str_ireplace(array('<br />','<br>','<br/>'), '', $text));
        $processedText = html_entity_decode($processedText, ENT_QUOTES | ENT_SUBSTITUTE);
        ...
        return [
            ...
            'time' => $militaryTime ? strftime("%H:%M", (int)$message->timeStamp) : strftime("%I:%M%p", (int)$message->timeStamp),
            ...
            'conversation' =>  $userId . '_' . ($message->senderId == $userId ? $message->recipientId : $message->senderId),
        ];
    }
    ...
    protected function getConversationPreviewText($userId, array $conversation)
    {
        ...
        return html_entity_decode(trim($eventData['short']), ENT_QUOTES | ENT_SUBSTITUTE);
    }
