...
class Users extends Base
{
    ...
    public function connect(SilexApplication $app)
    {
        ...
        $controllers->get('/{id}/', function (Request $request, SilexApplication $app, $id) {
            ...
            if (!in_array('viewChat', $relations)) {
                if ($loggedUserId != $id) {
            ...
            return $app->json($this->getUser($id, $loggedUserId, $relations));
        });
        ...
        // update password
        $controllers->put('/me/password/', function(Request $request) use ($app) {
            $loggedUserId = $app['users']->getLoggedUserId();
            $data = json_decode($request->getContent(), true);
            ...
            if (!$this->userService->isValidPassword($loggedUserId, $oldPassword)) {
                throw new BadRequestHttpException('Password is not correct');
            }
            ...
            {
                throw new BadRequestHttpException($language->text('base', 'reset_password_not_equal_error_message'));
            }

            if ( strlen(trim($password)) > UTIL_Validator::PASSWORD_MAX_LENGTH || strlen(trim($password)) < UTIL_Validator::PASSWORD_MIN_LENGTH )
            {
                throw new BadRequestHttpException(OW::getLanguage()->text('base', 'reset_password_length_error_message', array('min' => UTIL_Validator::PASSWORD_MIN_LENGTH, 'max' => UTIL_Validator::PASSWORD_MAX_LENGTH)));
            }

            $userDto = $this->userService->findUserById($loggedUserId);
            $userDto->setPassword($this->userService->hashPassword($password));
        ...
        $controllers->post('/', function(Request $request) use ($app) {
            ...
            $avatarRotate = 0;
            if (isset($data['avatarRotate'])) {
                $avatarRotate = $data['avatarRotate'];
                unset($data['avatarRotate']);
            }
            ...
            if ($data['avatarKey']) {
                ...
                $this->avatarService->createAvatar($user->id, true, false, $avatarRotate);
            }
            else {
