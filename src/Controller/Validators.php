...
class Validators extends Base
{
    ...
    public function connect(SilexApplication $app)
    {
        ...
        $controllers->post('/user-password/', function(Request $request) use ($app) {
            $vars = json_decode($request->getContent(), true);

            $password = !empty($vars['password']) ? $vars['password'] : null;
            ...
            if ($userDto && $this->userService->isValidPassword($userDto->id, $password)) {
                return $app->json([
                    'valid' => true
                ]);
            }
