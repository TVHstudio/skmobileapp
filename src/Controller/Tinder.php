        ...
        $controllers->get('/', function (Request $request, SilexApplication $app) {
            if (OW::getPluginManager()->isPluginActive('usearch')) {
                ...
                $excludeIds = $request->query->get('excludeIds')
                    ...
                    : [];
                // make sure the user has a permission for using filters
                $filter = $this->service->isPermissionAllowed($loggedUserId, 'skmobileapp', 'tinder_filters')
                ...
                if ($filter) {
                    $this->authService->trackActionForUser($loggedUserId, 'skmobileapp', 'tinder_filters');
                }

                $searchId = $this->service->tinderSearchUsers(
                    $loggedUserId, 
                    self::DEFAULT_USERS_LIMIT, 
