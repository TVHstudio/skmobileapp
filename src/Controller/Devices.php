...
class Devices extends Base
{
    ...
    public function connect(SilexApplication $app)
    {
        ...
        // unregister devices
        $controllers->delete('/', function (Request $request) use ($app) {
            $data = json_decode($request->getContent(), true);
            $userId = $app['users']->getLoggedUserId();

            if (!isset($data['deviceId']) || !is_string($data['deviceId'])) {
                return $app->json([], 400);
            }

            $device = $this->deviceService->findByUserIdAndUUid($userId, $data['deviceId']);
