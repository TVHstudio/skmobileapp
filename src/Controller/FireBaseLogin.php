    ...
    public function connect(SilexApplication $app)
    {
        ...
        $controllers->post('/login/', function(Request $request) use ($app) {
            if ($this->isPluginActive) {
                ...
                if (empty($vars['idToken'])) {
                    throw new BadRequestHttpException('Some important params are missing');
                }

                try {
                    list($action, $userId) = FIREBASEAUTH_BOL_Service::getInstance()->authenticateUser(
                        $vars['idToken'],
                        $vars['displayName'],
                        $vars['photoURL']
                    );
                    ...
                    $response['token'] = $app['security.jwt.encoder']->encode(
                        $this->service->getUserDataForToken($userId)
                    );
